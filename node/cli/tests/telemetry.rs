...
async fn telemetry_works() {
	...
	let mut node = process::Command::new(cargo_bin("deer-node"));
	...
	assert!(node.try_wait().unwrap().is_none(), "the process should still be running");

	// Stop the process
	kill(Pid::from_raw(node.id().try_into().unwrap()), SIGINT).unwrap();
	assert!(common::wait_for(&mut node, 40).map(|x| x.success()).unwrap_or_default());
