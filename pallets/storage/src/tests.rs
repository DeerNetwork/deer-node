...
macro_rules! assert_last_storage_event {
	($e:expr) => {
		assert_eq!(
...
fn stash_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_node_info!(2, deposit => stash_balance_x3);
...
fn withdraw_reserve_used_deposit() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(FileStorage::withdraw(Origin::signed(2)));
		assert_eq!(Nodes::<Test>::get(2).unwrap().deposit, stash_balance.saturating_add(10));
	})
}
...
fn report_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
            ...
            assert_last_storage_event!(crate::Event::NodeReported {
				controller: 2,
...
fn file_order_removed_if_file_size_is_small_than_actual_and_is_lack_fee() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_node_info!(2, deposit => default_stash_balance().saturating_add(10));
...
fn report_del_files() {
	ExtBuilder::default()
		...
		.execute_with(|| {
            ...
            assert_node_info!(2, deposit => default_stash_balance().saturating_sub(10), rid => 5, reported_at => 11, power => 9 * MB, used => 0, slash_used => 0);
			assert_eq!(
				RoundsReport::<Test>::get(CurrentRound::<Test>::get(), 2).unwrap(),
...
fn report_settle_files() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_node_info!(2, deposit => default_stash_balance().saturating_add(20));
...
fn report_settle_files_do_not_reward_unhealth_node() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let stash_balance = default_stash_balance();
            assert_node_info!(3, deposit => stash_balance.saturating_add(20));
            assert_node_info!(2, deposit => stash_balance, rid => 4, reported_at => 11, power => 10 * MB, used => 0, slash_used => MB);
			assert_ok!(MockData::new(4, 5, 10 * MB, &[]).report_data(3).call(2));
            assert_last_storage_event!(crate::Event::NodeReported {
				controller: 2,
				machine_id: MACHINES[3].get_machine_id(),
            ...
            assert_node_info!(2, deposit => stash_balance.saturating_sub(110), rid => 5, reported_at => 32, power => 10 * MB, used => 0, slash_used => 0);
		})
}
...
fn slash_offline() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_node_info!(2, deposit => default_stash_balance().saturating_sub(110));
