...
benchmarks_instance_pallet! {
	create_dutch {
		...
		T::Currency::make_free_balance_be(&owner, 1000_000u32.into());
		let (class, instance) = create_nft::<T, I>(&owner);
		let auction_id = NFTAuction::<T, I>::current_auction_id();
		let caller = owner.clone();
	}: _(SystemOrigin::Signed(caller.clone()), class, instance, 20u32.into(), 80u32.into(), 1200u32.into())
	verify {
	...
	bid_dutch {
		...
		let (class, instance) = create_nft::<T, I>(&owner);
		let auction_id = NFTAuction::<T, I>::current_auction_id();
		assert_ok!(NFTAuction::<T, I>::create_dutch(SystemOrigin::Signed(owner.clone()).into(), class, instance, 20u32.into(), 80u32.into(), 1200u32.into()));

		System::<T>::set_block_number(601u32.into());

		let caller: T::AccountId = account("bid", 0, SEED);
		whitelist_account!(caller);
		T::Currency::make_free_balance_be(&caller, 1000_000u32.into());
	...
	redeem_dutch {
		...
		System::<T>::set_block_number(662u32.into());
	...
	create_english {
		...
		let auction_id = NFTAuction::<T, I>::current_auction_id();
		let caller = owner.clone();
	}: _(SystemOrigin::Signed(caller.clone()), class, instance, 20u32.into(), 1u32.into(), 1200u32.into())
	verify {
	...
	bid_english {
		...
		let (class, instance) = create_nft::<T, I>(&owner);
		let auction_id = NFTAuction::<T, I>::current_auction_id();
		assert_ok!(NFTAuction::<T, I>::create_english(SystemOrigin::Signed(owner.clone()).into(), class, instance, 20u32.into(), 1u32.into(), 1200u32.into()));
		...
		whitelist_account!(caller);
		T::Currency::make_free_balance_be(&caller, 1000_000u32.into());
	}: _(SystemOrigin::Signed(caller.clone()), auction_id, 20u32.into())
	verify {
	...
	redeem_english {
		...
		whitelist_account!(caller);
		T::Currency::make_free_balance_be(&caller, 1000_000u32.into());
		assert_ok!(NFTAuction::<T, I>::bid_english(SystemOrigin::Signed(caller.clone()).into(), auction_id, 20u32.into()));
