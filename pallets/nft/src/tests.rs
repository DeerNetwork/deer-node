...
fn mint_should_work() {
	new_test_ext().execute_with(|| {
		Balances::make_free_balance_be(&1, 100);
		assert_ok!(NFT::create_class(Origin::signed(1), 0, vec![0, 0, 0], rate(5)));
		assert_ok!(NFT::mint(Origin::signed(1), 1, 0, 42, 2, vec![0, 0, 1], None, None));
		...
		assert_err!(
			NFT::mint(Origin::signed(1), 1, 0, 42, 1, vec![0, 0, 1], None, None),
			Error::<Test>::ConflictTokenId
		);
		assert_err!(
			NFT::mint(Origin::signed(1), 1, 1, 43, 1, vec![0, 0, 1], None, None),
			Error::<Test>::ClassNotFound
		);
		assert_err!(
			NFT::mint(Origin::signed(1), 1, 0, 43, 0, vec![0, 0, 1], None, None),
			Error::<Test>::InvalidQuantity
		);
		assert_err!(
			NFT::mint(Origin::signed(1), 1, 0, 43, 1, vec![0, 0, 1], Some(rate(21)), None),
			Error::<Test>::RoyaltyRateTooHigh
		);
		assert_err!(
			NFT::mint(Origin::signed(2), 2, 0, 43, 2, vec![0, 0, 1], None, None),
			Error::<Test>::NoPermission
		);

		assert_ok!(NFT::mint(Origin::signed(1), 2, 0, 52, 2, vec![0, 0, 1], None, None));
		...
		assert_ok!(NFT::mint(Origin::signed(1), 2, 0, 53, 2, vec![0, 0, 1], None, Some(1)));
...
fn burn_should_works() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(NFT::mint(Origin::signed(1), 1, 0, 43, 2, vec![0, 0, 1], None, None));
...
fn swap_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(NFT::create_class(Origin::signed(1), 0, vec![0, 0, 0], rate(5)));
		assert_ok!(NFT::mint(Origin::signed(1), 2, 0, 42, 2, vec![0, 0, 1], None, None));
